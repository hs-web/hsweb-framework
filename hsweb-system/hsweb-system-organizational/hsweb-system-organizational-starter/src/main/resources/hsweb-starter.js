...
function install(context) {
    ...
    database.createOrAlter("s_district")
        ...
        .addColumn().name("path").alias("path").comment("树路径,如: asb3-lsat").jdbcType(java.sql.JDBCType.VARCHAR).length(512).commit()
    ...
    database.createOrAlter("s_organization")
        ...
        .addColumn().name("path").alias("path").comment("树定位码").jdbcType(java.sql.JDBCType.VARCHAR).length(512).commit()
